...
public class NPOIExcelHelper
                    ...
                    //下面这一行是添加表头
                     ...
                     #region 列头及样式
                     {
                         ...
                         foreach (DataGridViewColumn column in myDgv.Columns)
                         {
                             ...
                             //设置列宽
                             sheet.SetColumnWidth(column.Index, (arrColWidth[column.Index] + 1) * 50);
                         }
                         // headerRow.Dispose();
                     }
                     #endregion
    ...
    public static MemoryStream DataGridViewToExcel(UIDataGridView myDgv, string strHeaderText)
    {
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = (HSSFSheet)workbook.CreateSheet();
        ...
        foreach (DataGridViewRow row in myDgv.Rows)
        {
            ...
            if (row.Index > 0)
            {
                foreach (DataGridViewColumn column in myDgv.Columns)
                {
                    ...
                    if (myDgv[column.Index, row.Index - 1].Value != null)
                    {
                        drValue = myDgv[column.Index, row.Index - 1].Value.ToString();
                    }
                    else
                    {
